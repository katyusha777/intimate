...
- **ID documents** (only when pending) via **short-TTL presigned GETs (≤5 min) from the dedicated private R2 bucket** (hard rule 3 / ARCHITECTURE §11 — R2, NOT Supabase Storage); our admin action is the only issuer, so issuance AND render both land in audit (`verification_doc_viewed`); overlay watermark "ADMIN VIEW · {admin} · {ts}" on the doc viewer (screenshot deterrent). Since 2026-08-23 a submission is **3 kind-labeled photos** (ID · selfie with ID · selfie with handwritten code) and the queue shows the **expected 4-letter code** (derived from her email) next to the viewer — the reviewer checks the ID reads, the face matches, and the paper matches the code. Approve/reject stamps the doc rows themselves (state/reviewer/date — the skeletal record that outlives the purge).
