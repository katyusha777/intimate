...
| `api/orgs` | `agencyBySlug(slug)` · `listAgencies()` — PUBLIC projection only (branch `locations` included; no contact/KvK/crawl config) · `createOrg(input)` · `joinFromConsent(input)` (the /agencies consent form: dedupe by site/email, consent stamp) | direct Drizzle on `orgs`; feeds `/{locale}/agencies/…` + the sitemap. Creation lives HERE (public form + admin both call it — the fence is one-directional); admin roster/crawl stays in `actions/admin/orgs.ts` |
| `api/prelaunch` | `listPrelaunchLeads()` — admin pre-signups tab, newest first · `leadContactsByEmail(email)` — onboarding contact prefill · `profilesForLeadEmails(emails)` — admin lead→profile match | direct Drizzle on `prelaunch_leads` (zero browser grants); read-only since launch 2026-08-21 — the landing (and its `addPrelaunchLead`) is gone, the leads remain real contacts |
