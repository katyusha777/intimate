...
DoD: all three states in kitchen-sink · toggle reachable without scrolling on every template · tab title/favicon flip verified (Playwright) · blur applies on cached/no-JS HTML the moment `html.safe-on` is present.

**Status: ✅ landed.** Safe mode is three-valued (`off | neutral | dev`); `neutral` now renders the **real photo heavily blurred** (CSS `blur`+scale on `html.safe-on`, stamped from the cookie before paint) — the visual privacy screen, not a gray box; `dev` is the operator's anime set. `SafeImage` ships the **real photo as `src`** (class `safe-img`) + `data-dev`/`data-real`. Footer/header carry the `SafeModeToggle` (cycles off→neutral→dev); while photos are blurred the Lightbox opens a turn-off dialog instead of the viewer (the floating mobile bubble was removed 2026-08-08 — it collided with the tab dock). Blog heroes route through `SafeImage`, so the blur covers articles too. The sitewide inline script neutralizes the tab title + favicon while on and restores them off, and adds the Esc·Esc boss key (flips off↔neutral). Kitchen-sink shows all three states. Design decision: the neutral set is generated (SVG gradients) rather than sourced — real discreet stock wasn't available, and generation reuses the existing deterministic-by-key FNV mechanism, keeping SSR/edge cache stable with zero test drift.
