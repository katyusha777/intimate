...
**Abuse walls, outermost first:** Cloudflare WAF/bot rules (never challenging the SEO/AI allowlist bots — SEO.md §1.3) → zone rate-limit on `/_actions/*` (INFRASTRUCTURE §7) → Turnstile on registration, contact-reveal, SMS start, first message of a thread → KV counters in actions (hourly budgets; the pattern in `aiSearch`/VERIFICATION.md §4). Scraping of public pages is accepted as physics (we're the fastest site in the market to scrape); Turnstile guards the *expensive* and *personal* actions, not reading.

## 6. Application security (the code itself)

- **Trust boundary = Zod.** Every action input, every cookie, every URL param, every AI/scraper output is strict-parsed before use (the codebase already does this — it is law, not style). Unknown fields dropped; lenient-parse only for AI outputs where partial results are acceptable (the `aiSearch` pattern).
