...
1. **Only anonymous, public content enters any shared cache** (Cache API, CDN): public HTML (KV page cache) and public live-profile photo bytes (`/media` — its media⟕profiles takedown gate runs BEFORE every cache lookup; private/owner-preview photos are `no-store`). Pages that vary by user (`/account`, `/admin`, `/messages`, anything reading the session cookie) send `Cache-Control: private, no-store` and are never `cache.put()`. Personalization on cached pages happens ONLY via server islands (ARCHITECTURE §2) — never by baking user state into cacheable HTML. **The registration wall (lib/gate.ts, 2026-08-23) runs in middleware strictly BEFORE the KV page-cache read**: anonymous humans are rewritten/redirected to the welcome pitch and never see (or seed) cached product HTML; post-wall the page cache is fed only by crawlers and the X-Warm cron, whose responses are the same public HTML — the anonymous-only cache invariant is unchanged. The wall is a conversion device, not a security boundary (a spoofed UA or forged sb-cookie only reaches public HTML crawlers get anyway).
