...
9. **The registration wall is a deliberate crawler exception (2026-08-23).** Anonymous HUMANS get only the `/{locale}/welcome/` pitch (rewritten in place on the home URLs; everything else 302s home) until they register — with profile pages exempt since 2026-08-24 (a professional's shared direct link must show her profile, not the wall); the crawlers in §1.2 + link-preview fetchers (UA match, `BOT_RE` in `src/lib/gate.ts`) and signed-in users get the full site, so indexability is unchanged. Keep `BOT_RE` in sync with the robots.txt list. Known risk, accepted: serving crawlers content humans can't see is cloaking by Google's definition — Bing/AI-first strategy prices that in; revisit if Google traffic ever matters more than registrations. Beyond that exception, bots get exactly what users get — never vary content by UA anywhere else.
