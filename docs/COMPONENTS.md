...
| Domain | Organisms |
|---|---|
| `profile/` | ✓ ProfileCard, ProfileDetail, ProfileHero, ProfileGallery, ProfilePhotoMosaic, ProfileSheet, ProfileFacts (the deal card), RatesTable, GoodToKnow, SimilarProfiles, ReceiptSheet (the trust receipt of dated verification facts opened by any verified mark), RequestSheet (the flagship three-tap pre-qualified request opened by the deal card's SEND MESSAGE), ReportControl, ProfileAdminBar (admin-session-only in-place moderation strip: state changes + single-photo takedown; every action audit-logged), RecentSignups (the live "recent signups" auto-advancing rail on the pre-launch landing; reuses ProfileCard variant=mini) — anticipated: ContactCard |
| `search/` | ✓ SearchListing, FilterSidebar |
| `auth/` | ✓ AuthModal — anticipated: PasswordReset, OtpForm |
| `dashboard/` | ✓ AccountShell, SignInPrompt, ProfileEditorForm, MediaManager, VerificationFlow, OnboardingChecklist (the get-your-profile-live journey card per ONBOARDING.md §3), StatusBanner (the in-review and you're-live states), BecomeAdvertiserBanner (client dashboard "did you mean to advertise?" self-service switch), ImportFromSiteBanner (paste-a-URL profile import above the editor: scrape+normalize preview via src/lib/import) — anticipated: ImportWizard steps, StatsTiles |
