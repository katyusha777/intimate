...
- Flow (2 photos, simplified 2026-08-27 — the paper-code selfie was a drop-off point and is retired; `code_selfie` survives only as a legacy enum value on old rows): advertiser uploads **her ID** and **a selfie holding the ID**. Each photo is EXIF-stripped client-side, re-stripped in the action, and written per-upload (kind-tagged `verification_docs` row) to a **separate R2 bucket used for nothing else**: EU jurisdiction, no public access, no r2.dev subdomain, no custom domain, no CORS — reachable exclusively through admin-authenticated server actions issuing short-lived presigned GETs for the review screen. Both kinds in → the account flips to `pending`.
