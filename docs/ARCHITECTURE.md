...
| Glue | Drizzle · Zod · Paraglide (nl/en/de/ro/it) · OpenRouter · Firecrawl · PostHog (analytics + errors, ANALYTICS.md) · OneSignal (web push, MESSAGING.md §8) | typed DB, validation, i18n strings, import extraction, errors |
...
- **Self-service import (shipped, `src/lib/import/`):** the advertiser (or the admin test tool) pastes HER OWN profile URL from a supported site (kinky.nl/sexjobs.nl/redlights.nl); one Firecrawl render + one OpenRouter pass normalizes it straight to our taxonomy (`ProfileEditSchema`), previewed then saved via the normal `saveProfile`. **Synchronous inline call — no queue/Cron/Realtime** (it returns in seconds); **photos are never imported** (she uploads her own, so she chooses). The `import_jobs` table + `/admin/imports` monitor remain for a future async bulk-seed.
- **Queues + Cron (future bulk-seed only):** batch import jobs writing progress rows that Realtime streams to the UI; scheduled expiry/cleanup.
...
**Launched 2026-08-21:** the apex serves the full site; the retired
`beta.intimate.nl` mirror 301s to it. Details: INFRASTRUCTURE.md §2.
...
- **Signaling reuses the existing stack — zero new services.** Peer negotiation (offer/answer, trickle ICE, glare, restarts) is delegated to **trystero core** (`@trystero-p2p/core`) through a custom strategy (`src/app/callroom.ts`) that rides Supabase Realtime **private channels** (RLS-authorized, §3) on `call:{id}:rtc` — the same authenticated client, not trystero's stock public-channel strategy. Call lifecycle is a `call_sessions` row with DB-trigger broadcasts on `call:{id}` ringing the client — the same `realtime.broadcast_changes()` pattern as everything else. Presence powers the "available for video call" badge.
...
- **Client:** vanilla-script dock in `organisms/call/` (CallDock — same discipline as Thread.astro). Incoming-call overlay via Realtime; mute/cam/end controls; ring timeout.
...
- Flow (3 photos, one screen each — restored + extended 2026-08-23): advertiser uploads **her ID**, **a selfie holding the ID**, and **a selfie holding a paper with her handwritten 4-letter code** (deterministic from her email, `lib/verification-code.ts` — no storage; the admin review derives and displays the same code). Each photo is EXIF-stripped client-side, re-stripped in the action, and written per-upload (kind-tagged `verification_docs` row) to a **separate R2 bucket used for nothing else**: EU jurisdiction, no public access, no r2.dev subdomain, no custom domain, no CORS — reachable exclusively through admin-authenticated server actions issuing short-lived presigned GETs for the review screen. All three kinds in → the account flips to `pending`.
...
Agencies are the supply multiplier (§ objective): one partner brings N profiles at once, they never import anything themselves (we crawl their site), and most agency sites are ancient — a free modern page is the recruiting pitch.

**SHIPPED 2026-08-11 (admin-managed, no agency login).** The built system is deliberately simpler than the original two-sided design below:
- **Data model:** the `orgs` table (NOT a new `agencies` table — DATA.md §1): slug, logo (R2 `org/…` via `/media`), site/contact info, description, KvK + verified flag, crawl config (`crawl_enabled`, `crawl_list_url`, `last_crawled_at`). Each org owns a placeholder `agency`-type `accounts` row (no auth user — upgradeable to a real login later) that holds its profiles; `profiles.org_id` is the single link, admin-assigned — no link-state machine, no mutual-consent flow (nobody but admins can link, so consent rides the partnership agreement).
- **Auto-crawl (the partner KVP):** admin sets a roster URL → discovery (Firecrawl render + LLM link filter, follows the listing's own pagination) → org-tagged `import_jobs` → per-URL agency extraction (shared taxonomy contract + name/age/photoUrls) → NEW profiles land `pending_review` with `pending_review` photos (EXIF-stripped via the Images transform binding, hard rule 2); re-crawls patch fields in place — the agency's site is the source of truth and wins over manual edits. Age gate (21+, hard rule 4) runs on every crawl. Engine: `app/data/db/crawl.ts`; cron tick every 5 min via workers/purge → `/api/crawl-tick`. See ADMIN.md §8.
- **Public page:** `/{locale}/agencies/{slug}/` (plural — the shipped URL) — logo, blurb, roster grid via `profilesApi.list({ orgId })`, full SEO treatment (§6): per-locale meta, canonical, hreflang, JSON-LD `Organization` + `ItemList`, sitemap.
- **Messaging:** agency profiles keep conversation mode default-OFF (nobody reads the placeholder account's inbox); contact goes through the profile's phone/WhatsApp buttons.

**Later (when agencies want logins):** self-service agency accounts (KvK business verification like §11), the dashboard scoped to their roster, mutual linking/handover for independent professionals, and the white-label "isolated island" — the agency's own (sub)domain serving their page as homepage via Cloudflare for SaaS custom hostnames mapped to `org_id` (likely paid; SEO rule to decide then: canonical to us or own identity, never both indexed).
