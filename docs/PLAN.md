...
This document is the foundation only: stack, how we use each piece to its absolute best, and non-negotiables. Feature plans and build order are decided as we go and live in GitHub Issues — not here.
...
| Runtime/edge | **Cloudflare Workers** | global compute + static assets, cache, KV, Queues, Cron, Images, Hyperdrive, Turnstile |
...
- **Storage:** one private encrypted bucket for verification documents only (see non-negotiables). Photos live in Cloudflare Images, not Supabase Storage.
...
3. **Verification documents are toxic waste:** private encrypted bucket, admin-only, **deleted after review** — retain only state/date/reviewer/hash. Never logged, never cached.
...
`staging` (Workers env + Supabase branch — all dev + migrations first) → `production` (after staging verification; Supabase MCP read-only vs prod). GitHub Issues is the plan of record for features; sessions stay scoped to one issue. Bun everywhere locally; CI: GitHub Actions running `bun install`, `bun test`, `bunx wrangler deploy`.
